...
impl<'a> NestingDepthVisitor<'a> {
    ...
    fn debug_visit_with(&self, method: &str, span: Span, code: bool, extra: Option<&str>) {
        ...
        if self
            ...
            .debug_span_info
            .as_ref()
            .is_some_and(|s| !s.intersects(&self.debug_span_info(span)))
        {
            return;
        }
...
impl<'a> NestingDepthVisitor<'a> {
    ...
    fn inc_if_else(&mut self, span: Span) {
        ...
        if self.config.debug {
            self.debug_visit_with("inc_if_else", span, false, Some(&format!("#{count}")));
        }
    }
    ...
    fn pop_context(&mut self) {
        ...
        if let Some(lint) = self.current_nesting_lint.take() {
            self.debug_visit("pop_context add nesting lint", ctx.span);
            self.lints.push(lint);
        }

        if let Some(lint) = ctx.consec_if_else_lint.take() {
            self.debug_visit("pop_context add if/else lint", ctx.span);
            self.lints.push(lint);
        }
    }
...
impl SpanInfo {
    fn intersects(&self, other: &SpanInfo) -> bool {
